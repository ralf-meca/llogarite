...
export function BudgetScreen({ invoices }: BudgetScreenProps) {
  ...
  return (
    <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent}>
      <Text style={styles.title}>Buxheti</Text>

      {!isLoading && target !== null && (
        <GlassView style={styles.card}>
          <Text style={styles.cardLabel}>Ky muaj</Text>
          <Text style={styles.amountText}>
            {formatAmount(spent)} <Text style={styles.amountTarget}>nga {formatAmount(target)}</Text>
          </Text>
          <View style={styles.progressWrapper}>
            <ProgressBar ratio={ratio} />
          </View>
          <Text style={styles.percentText}>{Math.round(ratio * 100)}% e buxhetit</Text>
        </GlassView>
      )}

      <GlassView style={styles.card}>
        <Text style={styles.cardLabel}>{target !== null ? 'Ndrysho buxhetin' : 'Vendos një buxhet mujor'}</Text>
        <GlassTextInput
          style={styles.input}
          placeholder="P.sh. 50'000"
